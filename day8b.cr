...
INPUT.each_line do |line|
    ...
    mapping = get_mapping(patterns.split(" "))
    nums = output.split(" ")
    digits = nums.map do |n|
        nset = Set.new(n.split(""))
        mapping[nset]
    end
...
def get_mapping(patterns)
    code_to_num = {} of Set(String) => Int32
    one = Set(String).new()
    seven = Set(String).new()
    four = Set(String).new()

    # identify unique amount of segments => numbers mappings
    patterns.each do |pat|
        case pat.size
        when 2 # it's a one
            one = Set.new(pat.split(""))
            code_to_num[one] = 1
        when 3 # it's a seven
            seven = Set.new(pat.split(""))
            code_to_num[seven] = 7
        when 4 # it's a four
            four = Set.new(pat.split(""))
            code_to_num[four] = 4
        when 5 # two, three or five
        when 6 # six, nine or zero
        when 7 # it's an eight
            eight = Set.new(pat.split(""))
            code_to_num[eight] = 8
        end
    end
    ...
    patterns.each do |pat|
        pat_set = Set.new(pat.split(""))
        case pat.size
        when 5 # two/three/five
            if (pat_set & seven).size == 3 # three and seven will have 3 seg in common
                code_to_num[pat_set] = 3
            elsif (pat_set & four).size == 2 # four and two will have 2 segs in common
                code_to_num[pat_set] = 2
            elsif (pat_set & four).size == 3 # four and five will have 3 segs in common
                code_to_num[pat_set] = 5
            end
        when 6 # six/nine/zero
            if (pat_set & one).size == 1 # one and six share 1 seg
                code_to_num[pat_set] = 6
            elsif (pat_set & four).size == 4 # four and nine share 4 segs
                code_to_num[pat_set] = 9
            else
                code_to_num[pat_set] = 0
            end
        end
    end
    code_to_num
